...
<template>
  <i-menu ref="sideMenu" :active-name="$route.name" :open-names="openNames" :theme="menuTheme" width="auto"
          @on-select="changeMenu">
    <template v-for="item in menuList">
      <i-menu-item v-if="item.children.length<1" :name="item.name" :key="'menuitem' + item.name">
        <i-icon :type="item.icon || item.icon" :size="iconSize" :key="'menuicon' + item.name"></i-icon>

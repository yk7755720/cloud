  ...
  <div class="disease-content">
    <i-form inline :label-width="80">
      <i-row>
        ...
        <i-form-item  inline label="案例名称">
          <i-input type="text" v-model="history.historyName" placeholder="请输入案例名称">
          </i-input>
        </i-form-item>

        <i-form-item prop="optId" inline label="术式">
          <i-input type="text" v-model="history.optName" placeholder="请输入术式名称">
          </i-input>
        </i-form-item>

        <i-form-item :label-width="20">
          <i-button type="primary" @click="getHistoryInfoList()" icon="ios-search">查询</i-button>
          <i-button type="primary" @click="addHistoryInfo()"  icon="plus">新增</i-button>
        </i-form-item>
      </i-row>
    </i-form>
    <div>
      <i-table border stripe :loading="false" :columns="historyInfoColumns" :data="historyInfoArray" class="disease_grid" ></i-table>
    </div>
    ...
    <i-modal width="700px"
      ...
      <i-form ref="editFormItem" :model="editFormItem" :label-width="80" :rules="ruleValidate" >
        ...
        <i-row>
          <i-col span="12">
            <i-form-item label="名称" prop="historyName">
              <i-input placeholder="请输入名称" v-model="editFormItem.historyName"  style="width:180px"></i-input>
            </i-form-item>
          </i-col>
        </i-row>
        ...
        <i-row>
          ...
          <i-form-item label="图片">
            <a @click="downloadResourceFile">{{editFormItem.coverUrl}}</a>
      ...
      <div slot="footer">
        <i-button type="primary" size="large" @click="saveHistoryInfo">保存</i-button>
  ...
  var jiaob= 0;
    ...
    export default {
              ...
              render: (h, params) => {
                ...
                btns.push(
                  h('i-button', {
                    ...
                    on: {
                      click: () => {
                        this.addHistoryInfo(params.row)
                      }
                    }
                  }, '编辑'),h('i-button', {
                    props: {
                    ...
                    on: {
                      click: () => {
                        this.delHistoryInfo(params.row)
                      }
                    }
                  }, '删除')
                )
      ...
      methods: {
        ...
        getHistoryInfoList (pageIndex) {
          if (isNaN(pageIndex)) {
            this.currentPage = 1
          }
          this.$axios.ajax({
            method: 'post',
            url: '/history/getHistoryInfoByCondition',
            data: {
              'pageNum': !isNaN(pageIndex) ? pageIndex : 1,
              'pageSize': this.pageSize,
              'historyName': this.history.historyName,
              'optName':this.history.optName
            }
          }).then((res) => {
            if (!!res.data.code && 10000 == res.data.code) {
              let data = res.data.data
              this.dataCount = !!data['total'] ? data['total'] : 0
              this.historyInfoArray = !!data['list'] ? data['list'] : []
            } else {
        ...
        delHistoryInfo(rows) {
          this.$Modal.confirm({
            ...
            onOk: () => {
              this.$axios.ajax({
                url: '/history/delHistoryInfo?id='+rows.id
              }).then((res) => {
                if (!!res.data.code && 10000 == res.data.code) {
        ...
        saveHistoryInfo () {
          ...
          this.$refs['editFormItem'].validate((valid) => {
            if (valid) {
              this.$axios.ajax({
                method: 'post',
                url: '/history/saveOrUpdateHistoryInfo',
                data:{
                  'id':this.editFormItem.id,
                  'historyName':this.editFormItem.historyName,
        ...
        uploadModels (response, file, fileList) {
          if (!!response.code && this.successCode == response.code) {
            if (Array.isArray(response.data) && response.data.length > 0) {
              this.editFormItem.coverUrl = response.data[0]['path'];
